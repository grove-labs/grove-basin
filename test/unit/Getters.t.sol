...
contract GroveBasinHarnessTests is GroveBasinTestBase {
    ...
    function test_getCollateralTokenValue() public view {
        assertEq(groveBasinHarness.getCollateralTokenValue(1, false), 1);
        assertEq(groveBasinHarness.getCollateralTokenValue(2, false), 2);
        assertEq(groveBasinHarness.getCollateralTokenValue(3, false), 3);

        assertEq(groveBasinHarness.getCollateralTokenValue(100e18, false), 100e18);
        assertEq(groveBasinHarness.getCollateralTokenValue(200e18, false), 200e18);
        assertEq(groveBasinHarness.getCollateralTokenValue(300e18, false), 300e18);

        assertEq(groveBasinHarness.getCollateralTokenValue(100_000_000_000e18, false), 100_000_000_000e18);
        assertEq(groveBasinHarness.getCollateralTokenValue(200_000_000_000e18, false), 200_000_000_000e18);
        assertEq(groveBasinHarness.getCollateralTokenValue(300_000_000_000e18, false), 300_000_000_000e18);
    }

    function testFuzz_getCollateralTokenValue(uint256 amount) public view {
        amount = _bound(amount, 0, 1e45);

        assertEq(groveBasinHarness.getCollateralTokenValue(amount, false), amount);
    }

    function test_getSwapTokenValue() public view {
        assertEq(groveBasinHarness.getSwapTokenValue(1, false), 1e12);
        assertEq(groveBasinHarness.getSwapTokenValue(2, false), 2e12);
        assertEq(groveBasinHarness.getSwapTokenValue(3, false), 3e12);

        assertEq(groveBasinHarness.getSwapTokenValue(100e6, false), 100e18);
        assertEq(groveBasinHarness.getSwapTokenValue(200e6, false), 200e18);
        assertEq(groveBasinHarness.getSwapTokenValue(300e6, false), 300e18);

        assertEq(groveBasinHarness.getSwapTokenValue(100_000_000_000e6, false), 100_000_000_000e18);
        assertEq(groveBasinHarness.getSwapTokenValue(200_000_000_000e6, false), 200_000_000_000e18);
        assertEq(groveBasinHarness.getSwapTokenValue(300_000_000_000e6, false), 300_000_000_000e18);
    }

    function testFuzz_getSwapTokenValue(uint256 amount) public view {
        amount = _bound(amount, 0, 1e45);

        assertEq(groveBasinHarness.getSwapTokenValue(amount, false), amount * 1e12);
    }
    ...
    function test_getAssetValue() public view {
        assertEq(groveBasinHarness.getAssetValue(address(swapToken), 1, false), groveBasinHarness.getSwapTokenValue(1, false));
        assertEq(groveBasinHarness.getAssetValue(address(swapToken), 2, false), groveBasinHarness.getSwapTokenValue(2, false));
        assertEq(groveBasinHarness.getAssetValue(address(swapToken), 3, false), groveBasinHarness.getSwapTokenValue(3, false));

        assertEq(groveBasinHarness.getAssetValue(address(swapToken), 1, true), groveBasinHarness.getSwapTokenValue(1, true));
        assertEq(groveBasinHarness.getAssetValue(address(swapToken), 2, true), groveBasinHarness.getSwapTokenValue(2, true));
        assertEq(groveBasinHarness.getAssetValue(address(swapToken), 3, true), groveBasinHarness.getSwapTokenValue(3, true));

        assertEq(groveBasinHarness.getAssetValue(address(swapToken), 1e6, false), groveBasinHarness.getSwapTokenValue(1e6, false));
        assertEq(groveBasinHarness.getAssetValue(address(swapToken), 2e6, false), groveBasinHarness.getSwapTokenValue(2e6, false));
        assertEq(groveBasinHarness.getAssetValue(address(swapToken), 3e6, false), groveBasinHarness.getSwapTokenValue(3e6, false));

        assertEq(groveBasinHarness.getAssetValue(address(swapToken), 1e6, true), groveBasinHarness.getSwapTokenValue(1e6, true));
        assertEq(groveBasinHarness.getAssetValue(address(swapToken), 2e6, true), groveBasinHarness.getSwapTokenValue(2e6, true));
        assertEq(groveBasinHarness.getAssetValue(address(swapToken), 3e6, true), groveBasinHarness.getSwapTokenValue(3e6, true));

        assertEq(groveBasinHarness.getAssetValue(address(collateralToken), 1, false), groveBasinHarness.getCollateralTokenValue(1, false));
        assertEq(groveBasinHarness.getAssetValue(address(collateralToken), 2, false), groveBasinHarness.getCollateralTokenValue(2, false));
        assertEq(groveBasinHarness.getAssetValue(address(collateralToken), 3, false), groveBasinHarness.getCollateralTokenValue(3, false));

        assertEq(groveBasinHarness.getAssetValue(address(collateralToken), 1, true), groveBasinHarness.getCollateralTokenValue(1, true));
        assertEq(groveBasinHarness.getAssetValue(address(collateralToken), 2, true), groveBasinHarness.getCollateralTokenValue(2, true));
        assertEq(groveBasinHarness.getAssetValue(address(collateralToken), 3, true), groveBasinHarness.getCollateralTokenValue(3, true));

        assertEq(groveBasinHarness.getAssetValue(address(collateralToken), 1e18, false), groveBasinHarness.getCollateralTokenValue(1e18, false));
        assertEq(groveBasinHarness.getAssetValue(address(collateralToken), 2e18, false), groveBasinHarness.getCollateralTokenValue(2e18, false));
        assertEq(groveBasinHarness.getAssetValue(address(collateralToken), 3e18, false), groveBasinHarness.getCollateralTokenValue(3e18, false));

        assertEq(groveBasinHarness.getAssetValue(address(collateralToken), 1e18, true), groveBasinHarness.getCollateralTokenValue(1e18, true));
        assertEq(groveBasinHarness.getAssetValue(address(collateralToken), 2e18, true), groveBasinHarness.getCollateralTokenValue(2e18, true));
        assertEq(groveBasinHarness.getAssetValue(address(collateralToken), 3e18, true), groveBasinHarness.getCollateralTokenValue(3e18, true));
    ...
    function testFuzz_getAssetValue(uint256 amount) public view {
        ...
        assertEq(groveBasinHarness.getAssetValue(address(swapToken),  amount, false), groveBasinHarness.getSwapTokenValue(amount, false));
        assertEq(groveBasinHarness.getAssetValue(address(swapToken),  amount, true),  groveBasinHarness.getSwapTokenValue(amount, true));
        assertEq(groveBasinHarness.getAssetValue(address(collateralToken),  amount, false), groveBasinHarness.getCollateralTokenValue(amount, false));
        assertEq(groveBasinHarness.getAssetValue(address(collateralToken),  amount, true),  groveBasinHarness.getCollateralTokenValue(amount, true));
...
contract GroveBasinAccrueFeeSharesTests is GroveBasinTestBase {
    ...
    function test_accrueFeeShares_zeroFeeShares() public {
        address feeClaimer_ = makeAddr("feeClaimer");
        groveBasinHarness.setFeeClaimerDirect(feeClaimer_);

        // Send tokens directly so totalAssets > 0 but totalShares == 0
        // This makes _convertToSharesRoundUp return ceilDiv(feeValue * 0, totalValue) == 0
        collateralToken.mint(address(groveBasinHarness), 1e18);

        assertEq(groveBasinHarness.totalShares(), 0);
        assertGt(groveBasinHarness.totalAssets(), 0);

        groveBasinHarness.accrueFeeShares(address(collateralToken), 1e18);

        assertEq(groveBasinHarness.shares(feeClaimer_), 0);
        assertEq(groveBasinHarness.totalShares(), 0);
    }
