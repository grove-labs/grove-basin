...
contract GroveBasinPreviewDeposit_SuccessTests is GroveBasinTestBase {
    ...
    function test_previewDeposit_secondaryToken_firstDeposit() public view {
        assertEq(groveBasin.previewDeposit(address(secondaryToken), 1), 1e12);
        assertEq(groveBasin.previewDeposit(address(secondaryToken), 2), 2e12);
        assertEq(groveBasin.previewDeposit(address(secondaryToken), 3), 3e12);

        assertEq(groveBasin.previewDeposit(address(secondaryToken), 1e6), 1e18);
        assertEq(groveBasin.previewDeposit(address(secondaryToken), 2e6), 2e18);
        assertEq(groveBasin.previewDeposit(address(secondaryToken), 3e6), 3e18);
    }

    function testFuzz_previewDeposit_secondaryToken_firstDeposit(uint256 amount) public view {
        amount = _bound(amount, 0, SECONDARY_TOKEN_MAX);
        assertEq(groveBasin.previewDeposit(address(secondaryToken), amount), amount * 1e12);
    }
    ...
    function test_previewDeposit_afterDepositsAndExchangeRateIncrease() public {
        ...
        _deposit(address(secondaryToken), depositor, 1e6);
        _assertOneToOne();
        ...
        assertEq(groveBasin.previewDeposit(address(secondaryToken),  1e6),  0.833333333333333333e18);
    ...
    function testFuzz_previewDeposit_afterDepositsAndExchangeRateIncrease(
        ...
        amount2        = _bound(amount2,        1,       SECONDARY_TOKEN_MAX);
        ...
        _deposit(address(secondaryToken), depositor, amount2);
        _assertOneToOne();
        ...
        uint256 secondaryTokenPreviewAmount = previewAmount / 1e12;

        assertEq(groveBasin.previewDeposit(address(collateralToken),  previewAmount),     previewAmount                           * totalSharesMinted / totalValue);
        assertEq(groveBasin.previewDeposit(address(secondaryToken),  secondaryTokenPreviewAmount), secondaryTokenPreviewAmount * 1e12                * totalSharesMinted / totalValue);  // Divide then multiply to replicate rounding
