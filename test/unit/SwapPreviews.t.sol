...
contract PSMPreviewSwapExactIn_FailureTests is GroveBasinTestBase {
    ...
    function test_previewSwapExactIn_bothSUsds() public {
        vm.expectRevert("GroveBasin/invalid-asset");
        groveBasin.previewSwapExactIn(address(susds), address(susds), 1);
    }
...
contract PSMPreviewSwapExactOut_FailureTests is GroveBasinTestBase {
    ...
    function test_previewSwapExactOut_bothSUsds() public {
        vm.expectRevert("GroveBasin/invalid-asset");
        groveBasin.previewSwapExactOut(address(susds), address(susds), 1);
    }
...
contract PSMPreviewSwapExactIn_UsdsAssetInTests is GroveBasinTestBase {
    ...
    function test_previewSwapExactIn_usdsToSUsds() public view {
        // Demo rounding down
        assertEq(groveBasin.previewSwapExactIn(address(usds), address(susds), 1e18 - 1), 0.8e18 - 1);
        assertEq(groveBasin.previewSwapExactIn(address(usds), address(susds), 1e18),     0.8e18);
        assertEq(groveBasin.previewSwapExactIn(address(usds), address(susds), 1e18 + 1), 0.8e18);

        assertEq(groveBasin.previewSwapExactIn(address(usds), address(susds), 1e18), 0.8e18);
        assertEq(groveBasin.previewSwapExactIn(address(usds), address(susds), 2e18), 1.6e18);
        assertEq(groveBasin.previewSwapExactIn(address(usds), address(susds), 3e18), 2.4e18);
    }
    ...
    function testFuzz_previewSwapExactIn_usdsToSUsds(uint256 amountIn, uint256 conversionRate) public {
        ...
        assertEq(groveBasin.previewSwapExactIn(address(usds), address(susds), amountIn), amountOut);
    }
...
contract PSMPreviewSwapExactOut_UsdsAssetInTests is GroveBasinTestBase {
    ...
    function test_previewSwapExactOut_usdsToSUsds() public view {
        // Demo rounding up
        assertEq(groveBasin.previewSwapExactOut(address(usds), address(susds), 1e18 - 1), 1.25e18 - 1);
        assertEq(groveBasin.previewSwapExactOut(address(usds), address(susds), 1e18),     1.25e18);
        assertEq(groveBasin.previewSwapExactOut(address(usds), address(susds), 1e18 + 1), 1.25e18 + 2);

        assertEq(groveBasin.previewSwapExactOut(address(usds), address(susds), 0.8e18), 1e18);
        assertEq(groveBasin.previewSwapExactOut(address(usds), address(susds), 1.6e18), 2e18);
        assertEq(groveBasin.previewSwapExactOut(address(usds), address(susds), 2.4e18), 3e18);
    }
    ...
    function testFuzz_previewSwapExactOut_usdsToSUsds(uint256 amountOut, uint256 conversionRate) public {
        ...
        uint256 amountIn = groveBasin.previewSwapExactOut(address(usds), address(susds), amountOut);
...
contract PSMPreviewSwapExactIn_USDCAssetInTests is GroveBasinTestBase {
    ...
    function test_previewSwapExactIn_usdcToSUsds() public view {
        // Demo rounding down
        assertEq(groveBasin.previewSwapExactIn(address(usdc), address(susds), 1e6 - 1), 0.799999e18);
        assertEq(groveBasin.previewSwapExactIn(address(usdc), address(susds), 1e6),     0.8e18);
        assertEq(groveBasin.previewSwapExactIn(address(usdc), address(susds), 1e6 + 1), 0.8e18);

        assertEq(groveBasin.previewSwapExactIn(address(usdc), address(susds), 1e6), 0.8e18);
        assertEq(groveBasin.previewSwapExactIn(address(usdc), address(susds), 2e6), 1.6e18);
        assertEq(groveBasin.previewSwapExactIn(address(usdc), address(susds), 3e6), 2.4e18);
    }
    ...
    function testFuzz_previewSwapExactIn_usdcToSUsds(uint256 amountIn, uint256 conversionRate) public {
        ...
        assertEq(groveBasin.previewSwapExactIn(address(usdc), address(susds), amountIn), amountOut);
    }
...
contract PSMPreviewSwapExactOut_USDCAssetInTests is GroveBasinTestBase {
    ...
    function test_previewSwapExactOut_usdcToSUsds() public view {
        // Demo rounding up
        assertEq(groveBasin.previewSwapExactOut(address(usdc), address(susds), 1e18 - 1), 1.25e6);
        assertEq(groveBasin.previewSwapExactOut(address(usdc), address(susds), 1e18),     1.25e6);
        assertEq(groveBasin.previewSwapExactOut(address(usdc), address(susds), 1e18 + 1), 1.25e6 + 1);

        assertEq(groveBasin.previewSwapExactOut(address(usdc), address(susds), 0.8e18), 1e6);
        assertEq(groveBasin.previewSwapExactOut(address(usdc), address(susds), 1.6e18), 2e6);
        assertEq(groveBasin.previewSwapExactOut(address(usdc), address(susds), 2.4e18), 3e6);
    }
    ...
    function testFuzz_previewSwapExactOut_usdcToSUsds(uint256 amountOut, uint256 conversionRate) public {
        ...
        uint256 amountIn = groveBasin.previewSwapExactOut(address(usdc), address(susds), amountOut);
    ...
    function test_demoRoundingUp_usdcToSUsds() public view {
        uint256 expectedAmountIn1 = groveBasin.previewSwapExactOut(address(usdc), address(susds), 0.8e18);
        uint256 expectedAmountIn2 = groveBasin.previewSwapExactOut(address(usdc), address(susds), 0.8e18 + 1);
        uint256 expectedAmountIn3 = groveBasin.previewSwapExactOut(address(usdc), address(susds), 0.8e18 + 0.8e12);
        uint256 expectedAmountIn4 = groveBasin.previewSwapExactOut(address(usdc), address(susds), 0.8e18 + 0.8e12 + 1);
...
contract PSMPreviewSwapExactIn_SUsdsAssetInTests is GroveBasinTestBase {

    function test_previewSwapExactIn_susdsToUsds() public view {
        // Demo rounding down
        assertEq(groveBasin.previewSwapExactIn(address(susds), address(usds), 1e18 - 1), 1.25e18 - 2);
        assertEq(groveBasin.previewSwapExactIn(address(susds), address(usds), 1e18),     1.25e18);
        assertEq(groveBasin.previewSwapExactIn(address(susds), address(usds), 1e18 + 1), 1.25e18 + 1);

        assertEq(groveBasin.previewSwapExactIn(address(susds), address(usds), 1e18), 1.25e18);
        assertEq(groveBasin.previewSwapExactIn(address(susds), address(usds), 2e18), 2.5e18);
        assertEq(groveBasin.previewSwapExactIn(address(susds), address(usds), 3e18), 3.75e18);
    }
    ...
    function testFuzz_previewSwapExactIn_susdsToUsds(uint256 amountIn, uint256 conversionRate) public {
        ...
        assertEq(groveBasin.previewSwapExactIn(address(susds), address(usds), amountIn), amountOut);
    }

    function test_previewSwapExactIn_susdsToUsdc() public view {
        // Demo rounding down
        assertEq(groveBasin.previewSwapExactIn(address(susds), address(usdc), 1e18 - 1), 1.25e6 - 1);
        assertEq(groveBasin.previewSwapExactIn(address(susds), address(usdc), 1e18),     1.25e6);
        assertEq(groveBasin.previewSwapExactIn(address(susds), address(usdc), 1e18 + 1), 1.25e6);

        assertEq(groveBasin.previewSwapExactIn(address(susds), address(usdc), 1e18), 1.25e6);
        assertEq(groveBasin.previewSwapExactIn(address(susds), address(usdc), 2e18), 2.5e6);
        assertEq(groveBasin.previewSwapExactIn(address(susds), address(usdc), 3e18), 3.75e6);
    }
    ...
    function testFuzz_previewSwapExactIn_susdsToUsdc(uint256 amountIn, uint256 conversionRate) public {
        ...
        assertEq(groveBasin.previewSwapExactIn(address(susds), address(usdc), amountIn), amountOut);
    }

}

contract PSMPreviewSwapExactOut_SUsdsAssetInTests is GroveBasinTestBase {

    function test_previewSwapExactOut_susdsToUsds() public view {
        // Demo rounding up
        assertEq(groveBasin.previewSwapExactOut(address(susds), address(usds), 1e18 - 1), 0.8e18);
        assertEq(groveBasin.previewSwapExactOut(address(susds), address(usds), 1e18),     0.8e18);
        assertEq(groveBasin.previewSwapExactOut(address(susds), address(usds), 1e18 + 1), 0.8e18 + 1);

        assertEq(groveBasin.previewSwapExactOut(address(susds), address(usds), 1.25e18), 1e18);
        assertEq(groveBasin.previewSwapExactOut(address(susds), address(usds), 2.5e18),  2e18);
        assertEq(groveBasin.previewSwapExactOut(address(susds), address(usds), 3.75e18), 3e18);
    }
    ...
    function testFuzz_previewSwapExactOut_susdsToUsds(uint256 amountOut, uint256 conversionRate) public {
        ...
        uint256 amountIn = groveBasin.previewSwapExactOut(address(susds), address(usds), amountOut);
    ...
    function test_previewSwapExactOut_susdsToUsdc() public view {
        // Demo rounding up
        assertEq(groveBasin.previewSwapExactOut(address(susds), address(usdc), 1e6 - 1), 0.8e18);
        assertEq(groveBasin.previewSwapExactOut(address(susds), address(usdc), 1e6),     0.8e18);
        assertEq(groveBasin.previewSwapExactOut(address(susds), address(usdc), 1e6 + 1), 0.800001e18);

        assertEq(groveBasin.previewSwapExactOut(address(susds), address(usdc), 1.25e6), 1e18);
        assertEq(groveBasin.previewSwapExactOut(address(susds), address(usdc), 2.5e6),  2e18);
        assertEq(groveBasin.previewSwapExactOut(address(susds), address(usdc), 3.75e6), 3e18);
    }
    ...
    function testFuzz_previewSwapExactOut_susdsToUsdc(uint256 amountOut, uint256 conversionRate) public {
        ...
        uint256 amountIn = groveBasin.previewSwapExactOut(address(susds), address(usdc), amountOut);
